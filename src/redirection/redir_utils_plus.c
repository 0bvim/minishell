...
/*   Updated: 2024/02/17 01:36:37 by vde-frei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_outfile(t_ast *node)
{
	if (node->type == R_REDIR)
		node->fd = append_trunc(node, TRUN);
	else if (node->type == APPEND)
		node->fd = append_trunc(node, APEN);
}
...
// 		while (read(node->fd, node->buff, 1))
// 			write(*file, node->buff, 1);
// 		close(node->fd);
