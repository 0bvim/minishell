...
CFILES += $(addprefix $(UTILS)/, str.c clear_console.c ft_lst_split.c panic.c which_token.c last_exit_status.c ft_clear_list.c ft_getenv.c redir_utils.c)
CFILES += $(addprefix $(CODEP)/, env_array.c)
CFILES += $(addprefix $(AST)/, ast.c ast_split_node.c ast_memory_handler.c search_tokens.c)
CFILES += $(addprefix $(EXEC)/, execution.c handling_pipe.c handling_redirs.c tokens_to_args.c)
